...
|---|---|---|
| `document-scanner` | DaemonSet (per node with USB scanner) | Stateless SANE driver: synchronous `POST /scan` writes raw TIFFs to the data volume. Grouping pages into documents, manifests and completion events belong to the processor (not yet built) |
...
| **smarter-device-manager** DaemonSet on every capture-equipped node, configured to expose `/dev/sr*`, `/dev/sg*` as Kubernetes resources | optical-ripper (`smarter-devices/sr0`, `smarter-devices/sg0`), and only when `hardware.enabled=false` | Without it, pod admission fails with `Insufficient smarter-devices/sr0` or similar. With `hardware.enabled=true` (the default) both hardware workloads reach their devices through hostPath `/dev` in the privileged `recognizer-hardware` namespace and claim no `smarter-devices/*` resources at all. The document-scanner never used it successfully: its `smarter-devices/bus-usb` claim named a resource smarter-device-manager does not advertise. |
