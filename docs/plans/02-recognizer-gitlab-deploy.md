...
docker login gitlab.orac.local:5050
docker pull gitlab.orac.local:5050/steve/recognizer/document-scanner:latest
docker inspect gitlab.orac.local:5050/steve/recognizer/document-scanner:latest | grep -E 'Architecture|Os'
```
...
```bash
helm pull oci://gitlab.orac.local:5050/steve/recognizer/charts/recognizer --version 0.0.1-rc.1
...
spec:
  ...
  containers:
    - name: probe
      image: gitlab.orac.local:5050/steve/recognizer/document-scanner:latest
      command: ["sleep", "30"]
  imagePullSecrets:
...
- Images republished as `gitlab.orac.local:5050/steve/recognizer/{document-scanner,notification-relay}`.
...
```bash
docker pull gitlab.orac.local:5050/steve/recognizer/document-scanner:0.1.0
helm pull oci://gitlab.orac.local:5050/steve/recognizer/charts/recognizer --version 0.1.0
```
...
Exit criteria: HelmRelease `READY: True` at `revision: 0.1.0`; pods running with images from `gitlab.orac.local:5050/steve/recognizer/*:0.1.0`.
...
- [ ] `docker pull gitlab.orac.local:5050/steve/recognizer/{document-scanner,notification-relay}:0.1.0` succeeds on a workstation authenticated to the GitLab registry
- [ ] `helm pull oci://gitlab.orac.local:5050/steve/recognizer/charts/recognizer --version 0.1.0` succeeds
